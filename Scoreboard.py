...
if __name__ == "__main__":
    ...
    for players in itertools.permutations(names, 2):
        #Create bot arrangement
        communicators = Communicator.create_bots(names=players*5)
        ids = range(10)
        random.shuffle(ids)
        states = {}
        ...
        while True:
            ...
            if all(x == player for x in bots[1:4]):
                add_points(player, 3)
                break
            state = tuple(bots)
            #Test for a tie
            if state in states:
                if states[state] is 3:
                    map(add_points, players, [1, 1])
                    break
                else:
                    states[state] += 1
            else:
